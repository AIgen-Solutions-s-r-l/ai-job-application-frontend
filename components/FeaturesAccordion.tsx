...
interface Feature {
  ...
  format?: string;
  alt?: string;
  embed?: boolean; // Nuevo atributo para identificar videos embebidos
  svg?: JSX.Element;
}
...
// Componente para los elementos del acordeón
const Item = ({
  feature,
  isOpen,
  setFeatureSelected,
...
// Componente para mostrar el contenido multimedia
const Media = ({ feature }: { feature: Feature }) => {
  const { type, path, format, alt, embed } = feature;
  ...
  if (type === "video") {
    if (embed) {
      // Renderiza videos embebidos como los de YouTube
      return (
        <iframe
          className={`${style}`}
          src={path}
