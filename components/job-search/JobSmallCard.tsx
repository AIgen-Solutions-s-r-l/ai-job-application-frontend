...
export const JobSmallCard: FC<Props> = ({ className, job, onClick }) => {
  ...
  const adjustedScore = Math.round(
    (1 - Math.acos(cosineScore) / (Math.PI / 1.8)) * 100
  );
  ...
  return (
    ...
    >
      <div className="flex gap-2 items-center md:mb-2">
        <p className="text-xs font-jura font-semibold">{finalScore}% Match</p>
      ...
      {job.country === 'Unknown' ? (
        ...
        <p className="text-sm md:text-base flex gap-2 items-center">
          <Image src={Pin} alt="pin" /> {`${job.city}, ${job.country}`} | {job.workplace_type}
        </p>
      )}
