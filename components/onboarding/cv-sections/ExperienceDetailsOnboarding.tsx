    ...
    <>
      {fields.map((responsibility, respIndex) => (
        <div key={responsibility.id} className="flex items-center gap-2 mb-2">
          ...
          />
         {errors.experienceDetails?.[index]?.key_responsibilities?.[respIndex] && (
                <p className="text-error mt-1">{errors.experienceDetails?.[index]?.key_responsibilities?.[respIndex].message}</p>
              )}
        </div>
      ))}
      ...
      {errors.experienceDetails?.[index]?.key_responsibilities?.root && <p className="text-error mt-1">Gay</p>}
    </>
  )
}
...
export const ExperienceDetailsOnboarding: FC = (): ReactElement => {
  ...
  const debouncedHandleSkillsChange = useCallback(
    debounce(handleSkillsChange, 500), // 300ms delay
    [handleSkillsChange] // Include handleSkillsChange in the dependency array
  );
  // const debouncedHandleSkillsChange = 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="md:col-span-2">
              ...
              {/* <textarea
                ...
                defaultValue={experience.skills_acquired?.join(', ')}
                placeholder="e.g., React, TypeScript, Node.js, Git (separate skills with commas)"
                className={`textarea textarea-bordered w-full h-24 ${
                  errors.experienceDetails?.[index]?.skills_acquired ? 'textarea-error' : ''
                }`}
              /> */}
              <textarea
